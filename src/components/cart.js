        ...
        <div className="ordering__cart_header">
          <div className="ordering__cart_header-cartImg">
            ...
            <span>
              {props.totalQuant < 0
                ? 'YOUR CART IS EMPTY'
                : '  YOUR CART:  ' + props.totalQuant + ' ITEM(S) '}
            </span>
          </div>
          <div
            className="ordering__cart_header-exit"
            onClick={() => Router.push('/')}
          >
            ...
            return (
              <>
                <div className="ordering__cart_content-name">
                  <span
                    className="ordering__cart_content-name_remove"
...
=======
                  ...
                  {item.size} {item.item}{' '}
                  {item.type == 'chicken' ? '(' + item.sauce + ')' : ''} x{' '}
                  {item.quant}
                </div>
                <div className="ordering__cart_content-price">
                  ${(item.price * item.quant).toFixed(2)}
                </div>
                <br />
>>>>>>> 30211e9333669660818c0e00dcf195c960be3ff6
              </>
            );
          })}
        </div>
        ...
        <div className="ordering__cart_total">
          <div className="ordering__cart_total-header">Tax:</div>
          ...
          <br />
          <div className="ordering__cart_total-header">TOTAL:</div>
          <div className="ordering__cart_total-content">
            $
            {props.subTotal >= 0
              ? (props.subTotal + props.subTotal * 0.06).toFixed(2)
              : (0.0).toFixed(2)}
          </div>
          <br />
          <br />
          <div
            className="ordering__orderButton"
            onClick={() =>
